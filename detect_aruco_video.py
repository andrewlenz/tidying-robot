...
async def update_frame(cam_src, vs, sender, cam_id, new_task, marker_dict, marker_params, robot):
	...
	while True:
		...
		if iter_count == 0:
			...
			dist_1 = new_task.euclidean_dist(new_task.coordinate_dict[new_task.robot1][1], new_task.coordinate_dict[object_allocate[new_task.robot1]][1])
		...
		if iter_count % 10 == 0:
			if len(new_task.object_order) > 0:
				print("Object allocate is ", object_allocate)
				...
				print("Intermediate angle is ",robot1_angle)
